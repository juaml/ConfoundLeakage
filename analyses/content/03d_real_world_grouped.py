...
for i_train, i_test in cv:
    ...
    rf_X = RandomForestClassifier(n_estimators=300, random_state=78).fit(X_sc_train, y_i_train)
    scores_raw.append(rf_X.score(X_sc_test, y_i_test))

    # Remove trained on class 0 only:
    # But transform both
    sc_0 = StandardScaler().fit(Xc_i_train_0)
    Xc_sc_train_0 = pd.DataFrame(sc_0.transform(Xc_i_train),
                                 columns=Xc_i_train.columns)

    Xc_sc_test_0 = pd.DataFrame(sc_0.transform(Xc_i_test),
                                columns=Xc_i_train.columns)
    ...
    rf_X = RandomForestClassifier(n_estimators=300, random_state=78).fit(Xc_rem_train, y_i_train)
    ...
    Xc_sc_train_shuffled_0 = pd.DataFrame(sc_0.transform(Xc_i_train_shuffled),
                                          columns=Xc_i_train_shuffled.columns)

    Xc_sc_test_0 = pd.DataFrame(sc_0.transform(Xc_i_test),
                                columns=Xc_i_train_shuffled.columns)

    remover = DataFrameConfoundRemover(confounds_match=confounds[0]).fit(Xc_sc_train_shuffled_0)
