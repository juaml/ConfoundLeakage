...
audio_data_shuffled_non_TaCo_folder = (f'{results_base}'
                                       'shuffled_features_non_TaCo/realistic_non_TaCo/models/')
...
model_raw = joblib.load(audio_data_non_TaCo_folder +
                        "audio_data_BDI___rf___False___716845___BDI.joblib")
model_rem = joblib.load(audio_data_non_TaCo_folder +
...
df_heat = pd.concat([
    pd.DataFrame(dict(raw_original=model_raw.named_steps.rf.feature_importances_,
                      )),
    pd.DataFrame(dict(rem_original=model_rem.named_steps.rf.feature_importances_,
                      )),
    pd.DataFrame(dict(raw_shuffled=model_raw_shuffled.named_steps.rf.feature_importances_,
                      )),
    pd.DataFrame(dict(rem_shuffled=model_rem_shuffled.named_steps.rf.feature_importances_,
                      )),
], axis=1)
...
# %% tags=["hide-input", "hide-output"]
fig, ax = plt.subplots()
sns.heatmap(df_feat_heat_raw[["raw_original"]],
            annot=df_feat_heat_raw[["rank_raw"]], fmt=".5g",
...
fig.savefig(
    './saved_figures/feature_importance_raw.svg'
)
fig.savefig(
    './saved_figures/feature_importance_raw.png'
)
glue('raw_feature_importance', fig)

fig, ax = plt.subplots()
sns.heatmap(df_feat_heat_rem[["rem_original"]],
            annot=df_feat_heat_rem[["rank_rem"]], fmt=".5g",
            vmin=vmin, vmax=vmax, ax=ax,
...
fig.savefig(
    './saved_figures/feature_importance_removed.svg'
)
fig.savefig(
    './saved_figures/feature_importance_remvoed.png'
)
glue('rem_feature_importance', fig)
# %% [markdown]
# Fit New DecisionTree only using most important removed feat
...
scores_raw, model_raw = run_cross_validation(
    X=most_imp_feat_rem, y=y, data=df_original,
    model=DecisionTreeClassifier(),
    preprocess_X=["zscore"], return_estimator="final",
    seed=42

)
scores_rm, model_rm = run_cross_validation(
    X=most_imp_feat_rem, confounds=confounds, y=y, data=df_original,
    model=DecisionTreeClassifier(),
    preprocess_X=["zscore", "remove_confound"],
    return_estimator="final", seed=42
)
print("RM:", scores_rm["test_score"].mean(), "Raw:", scores_raw["test_score"].mean())

# preprocess Features
Xcr, _ = model_rm.preprocess(
    df_original[most_imp_feat_rem+confounds],
    df_original[y])

Xraw, _ = model_raw.preprocess(
    df_original[most_imp_feat_rem],
    df_original[y])
...
DecisionBoundaryDisplay.from_estimator(
    ...
    cmap=(LinearSegmentedColormap
          .from_list("MyCmapName", ["#e35d6d", "#3498eb"])
          ),
    alpha=.5
)
...
fig_raw_decision.savefig(
    './saved_figures/decision_boundary_feature_importance_raw.svg'
)
fig_raw_decision.savefig(
    './saved_figures/decision_boundary_feature_importance_raw.png'
)
...
# %% [markdown]
# DecisionBoundaries of DT trained on 2 most important
# Features.
# %% using original data [tabbed]
# ```{glue:figure} raw_decision
# :figwidth: 900px
