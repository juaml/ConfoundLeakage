...
target = np.random.normal(scale=.2, size=2000) + confound
n_feat = 1
...
for i in feat_nbr:
    n_feat = i
    feat = np.append(
        chi2.rvs(df=3, scale=.5, size=[1000, n_feat]),
    ...
    scores_raw = run_cross_validation(
        X=X, y='target', data=df,
        model='rf', preprocess_X=['zscore'],
        cv=RepeatedKFold(),
    ...
    scores_rem = run_cross_validation(
        X=X, y='target', data=df, confounds=['conf'],
    ...
    save_paper_val(increase_features_extra_save_paper,
...
df_no_rem = pd.DataFrame(dict(scores=all_scores_raw,
                              iteration=feat_nbr,
                              yerr=all_yerr_raw
                              ))
...
g.set(xlabel='Number Of Features', ylabel='R²',
      title='χ2 Distributed Confounded Features')
