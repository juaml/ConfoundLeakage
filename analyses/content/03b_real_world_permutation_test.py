...
# %% tags=["hide-input", "hide-output"]
#  Compute mean scores of CV for each repeat of the cross_validation run
df_scores = pd.concat([pd.read_csv(results_base + "/" + i)
                       for i in os.listdir(results_base)])
df_CV_results = (df_scores
                 .groupby(["confound_removed", "shuffled", "repeat"])
                 .agg("mean")
                 .reset_index()
                 .drop(columns=["fold"])
                 )
...
# %% tags=["hide-input", "hide-output"]
score_raw = df_CV_results.query("(shuffled==False) & (confound_removed == False)").score.values
...
C_raw = (df_CV_results.query("(shuffled==True) & (confound_removed == False)").score
         >= score_raw).sum()
pval_raw = compute_pval(C_raw, n_permutations)

score_rem = df_CV_results.query("(shuffled==False) & (confound_removed == True)").score.values
assert len(score_rem) == 1
score_rem = score_rem[0]
C_rem = (df_CV_results.query("(shuffled==True) & (confound_removed == True)").score
         >= score_rem).sum()
pval_rem = compute_pval(C_rem, n_permutations)

# pvals are significant (threshold .05) or not?
significance = ["*" if x < .05 else "n.s."
                for x in [pval_raw, pval_rem]
                ]
...
df_plot = (
    df_CV_results
    .assign(
        confound_removed=lambda df: df.confound_removed.map(lambda x: "$X_{CR}$" if x else "X"),
...
ax.plot([-.4, 0], [score_raw, score_raw], zorder=100, c=red)
ax.plot([.6, 1], [score_rem, score_rem], zorder=100, c=red)
...
annotator = Annotator(ax, pairs,
                      data=df_plot, x="Features", hue="Were Shuffled?", y="score")
